...
$(document).ready(function () {
    ...
    function changeHeader(ele) { /*Two way linking.*/
...
function updateCell(property, value, defaultPossible) {
    $(".input-cell.selected").each(function () {
        ...
        let [rowId, colId] = getRowCol(this);
        if (cellData[selectedSheet][rowId]) {/*cell exist only update*/
            if (cellData[selectedSheet][rowId][colId]) {
                cellData[selectedSheet][rowId][colId][property] = value;
            } else {/*cell does not exist create*/
...
function emptuSheet() { /*Empty sheet */
...
function loadSheet() {/* load Sheet after empty*/
    let sheetInfo = cellData[selectedSheet];
    for (let i of Object.keys(sheetInfo)) {
...
function addSheetEvents() {
    ...
    $(".sheet-tab.selected").contextmenu(function (e) {/*right click modal */
        e.preventDefault();
        ...
        if ($(".sheet-options-modal").length == 0) {
            ...
            $(".sheet-rename").click(function () {
                ...
                $(".submit-button").click(function () {
                    ...
                    $(".sheet-tab.selected").text(newSheetName);/*change in UI */
                    let newCellData = {};
...
function selectSheet(ele) {
    ...
    emptuSheet();
    selectedSheet = $(ele).text(); /*sheet name */
    loadSheet();
}
let selectedCells = [];
let cut = false;
...
$(".icon-paste").click(function () {
    ...
    for (let cell of selectedCells) {
        ...
        if (!cellData[selectedSheet][newRowId]) {//row not already exist..
